<div class="d-flex mb-2">
    <div id="search-sort-section" class="form-inline mr-auto">
        ...
        <div>
            <select wire:model='orderBy' class="form-control form-group" style="width: 150px;">
                ...
                <option value='manager_id'>{{ __('work.manager') }}</option>

                <option value='department_id'>{{ __('work.department') }}</option>

                <option value='user_id'>{{ __('work.user') }}</option>

                <option value='job_title'>{{ __('work.job_title') }}</option>
    ...
    <div class="form-inline">
        @role('owner|operations')
            <div>
                <button type="button" class="mr-1 btn btn-warning" data-toggle="modal" data-target="#filter-work-modal">
        ...
        @role('owner|operations')
            <div>
                <button type="button" class="btn btn-info" data-toggle="modal" data-target="#show-hide-work-columnModal">
        ...
        @if ($admin_view_status != 'deleted')
            ...
            @permission('export-excel-work')
                <div>
                    <a class="mr-1 btn btn-warning" target="_blank"
                        href="{{ route('work.export', ['by_date' => $byDate, 'from_date' => $fromDate, 'to_date' => $toDate]) }}">
        ...
        @if ($admin_view_status != 'deleted')
            @permission('create-work')
                <div>
                    <button type="button" class="btn btn-success" data-toggle="modal" data-target="#create-new-work-modal">
    ...
    <div wire:ignore.self class="modal fade" id="show-hide-work-columnModal" tabindex="-1"
        aria-labelledby="show-hide-work-columnModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="show-hide-work-columnModalLabel">
                ...
                <div class="modal-body">
                    <div>
                        ...
                        @for ($i = 0; $i < count($showColumn); $i++)
                            ...
                            @if (strlen(__('work.' . $show_kye)) > 2)
                                <div class="form-check">
                                    <input wire:model="showColumn.{{ $kye }}" class="form-check-input"
                                        type="checkbox" value="true" id="filter-work-{{ $kye }}">
                                    <label class="form-check-label" for="filter-work-{{ $kye }}">
                                        {{ __('work.' . $show_kye) }}
                                    </label>
                                </div>
                            @endif
    ...
    @if ($admin_view_status != 'deleted')
        @include('livewire.work.work-create-modal')
    @endif
        ...
        <div wire:ignore.self class="modal fade" id="import-data" tabindex="-1" aria-labelledby="import-dataLabel"
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form action="{{ route('work.import') }}" enctype="multipart/form-data" method="post"
    ...
    <div wire:ignore.self class="modal fade" id="filter-work-modal" tabindex="-1"
        aria-labelledby="filter-work-modalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="filter-work-modalLabel">
                        {{ __('global.filter') }}
                    </h5>
                ...
                <div class="modal-body">
                    ...
                    <div class='form-group'>
                        <label for='department-select'>{{ __('global.by_departments') }}</label>
                        ...
                        @foreach ($departments as $department)
                            @if (in_array($department->id, $filter_departments_id))
                                <div class='form-check form-check-inline'>
                                    <input wire:model='filter_departments_id' class='form-check-input'
