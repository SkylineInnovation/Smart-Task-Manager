<div class="d-flex mb-2">
    <div id="search-sort-section" class="form-inline mr-auto">
        ...
        <div>
            <select wire:model='orderBy' class="form-control form-group" style="width: 150px;">
                ...
                <option value='user_id'>{{ __('userdetail.user') }}</option>

                <option value='title'>{{ __('userdetail.title') }}</option>

                <option value='nationality'>{{ __('userdetail.nationality') }}</option>

                <option value='id_number'>{{ __('userdetail.id_number') }}</option>

                <option value='address'>{{ __('userdetail.address') }}</option>

                <option value='qualification'>{{ __('userdetail.qualification') }}</option>

                <option value='salary'>{{ __('userdetail.salary') }}</option>

                <option value='home'>{{ __('userdetail.home') }}</option>

                <option value='transport'>{{ __('userdetail.transport') }}</option>

                <option value='branch_id'>{{ __('userdetail.branch') }}</option>
    ...
    <div class="form-inline">
        @role('owner|operations')
            <div>
                <button type="button" class="mr-1 btn btn-warning" data-toggle="modal"
                    data-target="#filter-userdetail-modal">
        ...
        @role('owner|operations')
            <div>
                <button type="button" class="btn btn-info" data-toggle="modal"
                    data-target="#show-hide-userdetail-columnModal">
        ...
        @if ($admin_view_status != 'deleted')
            @permission('import-excel-userdetail')
                <div>
            ...
            @permission('export-excel-userdetail')
                <div>
                    <a class="mr-1 btn btn-warning" target="_blank"
                        href="{{ route('userdetail.export', ['by_date' => $byDate, 'from_date' => $fromDate, 'to_date' => $toDate]) }}">
            ...
            @permission('create-userdetail')
                <div>
                    <button type="button" class="btn btn-success" data-toggle="modal"
                        data-target="#create-new-userdetail-modal">
    ...
    <div wire:ignore.self class="modal fade" id="show-hide-userdetail-columnModal" tabindex="-1"
        aria-labelledby="show-hide-userdetail-columnModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="show-hide-userdetail-columnModalLabel">
                ...
                <div class="modal-body">
                    <div>
                        ...
                        @for ($i = 0; $i < count($showColumn); $i++)
                            ...
                            @if (strlen(__('userdetail.' . $show_kye)) > 2)
                                <div class="form-check">
                                    <input wire:model="showColumn.{{ $kye }}" class="form-check-input"
                                        type="checkbox" value="true" id="filter-userdetail-{{ $kye }}">
                                    <label class="form-check-label" for="filter-userdetail-{{ $kye }}">
                                        {{ __('userdetail.' . $show_kye) }}
                                    </label>
                                </div>
                            @endif
                        @endfor
    ...
    @if ($admin_view_status != 'deleted')
        @include('livewire.userdetail.userdetail-create-modal')
    @endif
        ...
        <div wire:ignore.self class="modal fade" id="import-data" tabindex="-1" aria-labelledby="import-dataLabel"
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form action="{{ route('userdetail.import') }}" enctype="multipart/form-data" method="post"
    ...
    <div wire:ignore.self class="modal fade" id="filter-userdetail-modal" tabindex="-1"
        aria-labelledby="filter-userdetail-modalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="filter-userdetail-modalLabel">
                        {{ __('global.filter') }}
                    </h5>
