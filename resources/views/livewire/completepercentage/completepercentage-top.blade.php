<div class="d-flex mb-2">
    {{-- <div id="search-sort-section" class="form-inline mr-auto">
        ...
        <div>
            <select wire:model='orderBy' class="form-control form-group" style="width: 150px;">
                ...
                <option value='task_id'>{{ __('completepercentage.task') }}</option>

                <option value='user_id'>{{ __('completepercentage.user') }}</option>

                <option value='percentage'>{{ __('completepercentage.percentage') }}</option>

                <option value='rate_text'>{{ __('completepercentage.rate_text') }}</option>

                <option value='rate_val'>{{ __('completepercentage.rate_val') }}</option>
    ...
    <div class="form-inline">
        {{-- @role('owner|operations')
            <div>
                <button type="button" class="mr-1 btn btn-warning" data-toggle="modal"
                    data-target="#filter-completepercentage-modal">
        ...
        @role('owner|operations')
            <div>
                <button type="button" class="btn btn-info" data-toggle="modal"
                    data-target="#show-hide-completepercentage-columnModal">
        ...
        {{-- @if ($admin_view_status != 'deleted')
            @permission('import-excel-completepercentage')
                <div>
            ...
            @permission('export-excel-completepercentage')
                <div>
                    <a class="mr-1 btn btn-warning" target="_blank"
                        href="{{ route('completepercentage.export', ['by_date' => $byDate, 'from_date' => $fromDate, 'to_date' => $toDate]) }}">
            ...
            @permission('create-completepercentage')
                <div>
                    <button type="button" class="btn btn-success" data-toggle="modal"
                        data-target="#create-new-completepercentage-modal">
    ...
    {{-- <div wire:ignore.self class="modal fade" id="show-hide-completepercentage-columnModal" tabindex="-1"
        aria-labelledby="show-hide-completepercentage-columnModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="show-hide-completepercentage-columnModalLabel">
                ...
                <div class="modal-body">
                    <div>
                        ...
                        @for ($i = 0; $i < count($showColumn); $i++)
                            ...
                            @if (strlen(__('completepercentage.' . $show_kye)) > 2)
                                <div class="form-check">
                                    <input wire:model="showColumn.{{ $kye }}" class="form-check-input"
                                        type="checkbox" value="true"
                                        id="filter-completepercentage-{{ $kye }}">
                                    <label class="form-check-label" for="filter-completepercentage-{{ $kye }}">
                                        {{ __('completepercentage.' . $show_kye) }}
                                    </label>
                                </div>
                            @endif
                        @endfor
    ...
    @if ($admin_view_status != 'deleted')
        @include('livewire.completepercentage.completepercentage-create-modal')
    @endif
        ...
        <div wire:ignore.self class="modal fade" id="import-data" tabindex="-1" aria-labelledby="import-dataLabel"
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form action="{{ route('completepercentage.import') }}" enctype="multipart/form-data" method="post"
    ...
    {{-- <div wire:ignore.self class="modal fade" id="filter-completepercentage-modal" tabindex="-1"
        aria-labelledby="filter-completepercentage-modalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="filter-completepercentage-modalLabel">
                        {{ __('global.filter') }}
                    </h5>
