<div class="d-flex mb-2">
    <div id="search-sort-section" class="form-inline mr-auto">
        ...
        <div>
            <select wire:model='orderBy' class="form-control form-group">
                ...
                <option value='user_id'>{{ __('devicetokenlist.user') }}</option>

                <option value='device_info'>{{ __('devicetokenlist.device_info') }}</option>

                <option value='device_type'>{{ __('devicetokenlist.device_type') }}</option>

                <option value='application'>{{ __('devicetokenlist.application') }}</option>

                <option value='device_token'>{{ __('devicetokenlist.device_token') }}</option>
    ...
    <div class="form-inline">
        @role('owner|operations')
            <div>
                <button type="button" class="mr-1 btn btn-warning" data-toggle="modal"
                    data-target="#filter-devicetokenlist-modal">
        ...
        @role('owner|operations')
            <div>
                <button type="button" class="btn btn-info" data-toggle="modal"
                    data-target="#show-hide-devicetokenlist-columnModal">
        ...
        @if ($admin_view_status != 'deleted')
            @permission('import-excel-devicetokenlist')
                <div>
            ...
            @permission('export-excel-devicetokenlist')
                <div>
                    <a class="mr-1 btn btn-warning" target="_blank"
                        href="{{ route('devicetokenlist.export', ['by_date' => $byDate, 'from_date' => $fromDate, 'to_date' => $toDate]) }}">
            ...
            @permission('create-devicetokenlist')
                <div>
                    <button type="button" class="btn btn-success" data-toggle="modal"
                        data-target="#create-new-devicetokenlist-modal">
    ...
    <div wire:ignore.self class="modal fade" id="show-hide-devicetokenlist-columnModal" tabindex="-1"
        aria-labelledby="show-hide-devicetokenlist-columnModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="show-hide-devicetokenlist-columnModalLabel">
                ...
                <div class="modal-body">
                    <div>
                        ...
                        @for ($i = 0; $i < count($showColumn); $i++)
                            ...
                            @if (strlen(__('devicetokenlist.' . $show_kye)) > 2)
                                <div class="form-check">
                                    <input wire:model="showColumn.{{ $kye }}" class="form-check-input"
                                        type="checkbox" value="true" id="filter-devicetokenlist-{{ $kye }}">
                                    <label class="form-check-label" for="filter-devicetokenlist-{{ $kye }}">
                                        {{ __('devicetokenlist.' . $show_kye) }}
                                    </label>
                                </div>
                            @endif
                        @endfor
    ...
    @if ($admin_view_status != 'deleted')
        @include('livewire.devicetokenlist.devicetokenlist-create-modal')
    @endif
        ...
        <div wire:ignore.self class="modal fade" id="import-data" tabindex="-1" aria-labelledby="import-dataLabel"
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form action="{{ route('devicetokenlist.import') }}" enctype="multipart/form-data" method="post"
    ...
    <div wire:ignore.self class="modal fade" id="filter-devicetokenlist-modal" tabindex="-1"
        aria-labelledby="filter-devicetokenlist-modalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="filter-devicetokenlist-modalLabel">
                        {{ __('global.filter') }}
                    </h5>
