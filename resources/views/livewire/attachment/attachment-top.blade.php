<div class="d-flex mb-2">
    <div id="search-sort-section" class="form-inline mr-auto">
        ...
        <div>
            <select wire:model='orderBy' class="form-control form-group" style="width: 150px;">
                ...
                <option value='user_id'>{{ __('attachment.user') }}</option>

                <option value='task_id'>{{ __('attachment.task') }}</option>

                <option value='title'>{{ __('attachment.title') }}</option>

                <option value='desc'>{{ __('attachment.desc') }}</option>

                <option value='file'>{{ __('attachment.file') }}</option>

                <option value='main_attachment_id'>{{ __('attachment.main_attachment') }}</option>
    ...
    <div class="form-inline">
        @role('owner|operations')
            <div>
                <button type="button" class="mr-1 btn btn-warning" data-toggle="modal"
                    data-target="#filter-attachment-modal">
        ...
        @role('owner|operations')
            <div>
                <button type="button" class="btn btn-info" data-toggle="modal"
                    data-target="#show-hide-attachment-columnModal">
        ...
        {{-- @if ($admin_view_status != 'deleted')
            @permission('import-excel-attachment')
                <div>
            ...
            @permission('export-excel-attachment')
                <div>
                    <a class="mr-1 btn btn-warning" target="_blank"
                        href="{{ route('attachment.export', ['by_date' => $byDate, 'from_date' => $fromDate, 'to_date' => $toDate]) }}">
            ...
            @permission('create-attachment')
                <div>
                    <button type="button" class="btn btn-success" data-toggle="modal"
                        data-target="#create-new-attachment-modal">
    ...
    <div wire:ignore.self class="modal fade" id="show-hide-attachment-columnModal" tabindex="-1"
        aria-labelledby="show-hide-attachment-columnModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="show-hide-attachment-columnModalLabel">
                ...
                <div class="modal-body">
                    <div>
                        ...
                        @for ($i = 0; $i < count($showColumn); $i++)
                            ...
                            @if (strlen(__('attachment.' . $show_kye)) > 2)
                                <div class="form-check">
                                    <input wire:model="showColumn.{{ $kye }}" class="form-check-input"
                                        type="checkbox" value="true" id="filter-attachment-{{ $kye }}">
                                    <label class="form-check-label" for="filter-attachment-{{ $kye }}">
                                        {{ __('attachment.' . $show_kye) }}
                                    </label>
                                </div>
                            @endif
                        @endfor
    ...
    @if ($admin_view_status != 'deleted')
        @include('livewire.attachment.attachment-create-modal')
    @endif
        ...
        <div wire:ignore.self class="modal fade" id="import-data" tabindex="-1" aria-labelledby="import-dataLabel"
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form action="{{ route('attachment.import') }}" enctype="multipart/form-data" method="post"
    ...
    <div wire:ignore.self class="modal fade" id="filter-attachment-modal" tabindex="-1"
        aria-labelledby="filter-attachment-modalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="filter-attachment-modalLabel">
                        {{ __('global.filter') }}
                    </h5>
                ...
                <div class="modal-body">
                    ...
                    {{-- <div class='form-group'>
                        <label for='main_attachment-select'>{{ __('global.by_main_attachments') }}</label>
                        <select id='main_attachment-select' class='form-control' wire:model='select_main_attachment'>
                            <option>{{ __('global.select-main_attachments') }}</option>
                            @foreach ($main_attachments as $main_attachment)
                                <option value='{{ $main_attachment->id }}'>{{ $main_attachment->crud_name() }}
                                </option>
                            @endforeach
                        </select>

                        @foreach ($main_attachments as $main_attachment)
                            @if (in_array($main_attachment->id, $filter_main_attachments_id))
                                <div class='form-check form-check-inline'>
                                    <input wire:model='filter_main_attachments_id' class='form-check-input'
                                        type='checkbox' value='{{ $main_attachment->id }}'
                                        id='filter-main_attachments-id-{{ $main_attachment->id }}'>
                                    <label class='form-check-label'
                                        for='filter-main_attachments-id-{{ $main_attachment->id }}'>
                                        {{ $main_attachment->crud_name() }}
                                    </label>
                                </div>
                            @endif
                        @endforeach
