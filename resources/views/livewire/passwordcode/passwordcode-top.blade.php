<div class="d-flex mb-2">
    <div id="search-sort-section" class="form-inline mr-auto">
        ...
        <div>
            <select wire:model='orderBy' class="form-control form-group" style="width: 150px;">
                ...
                <option value='user_id'>{{ __('passwordcode.user') }}</option>

                <option value='code'>{{ __('passwordcode.code') }}</option>

                <option value='note'>{{ __('passwordcode.note') }}</option>

                <option value='is_used'>{{ __('passwordcode.is_used') }}</option>

                <option value='ip_address'>{{ __('passwordcode.ip_address') }}</option>
    ...
    <div class="form-inline">
        @role('owner|operations')
            <div>
                <button type="button" class="mr-1 btn btn-warning" data-toggle="modal"
                    data-target="#filter-passwordcode-modal">
        ...
        @role('owner|operations')
            <div>
                <button type="button" class="btn btn-info" data-toggle="modal"
                    data-target="#show-hide-passwordcode-columnModal">
        ...
        @if ($admin_view_status != 'deleted')
            ...
            @permission('export-excel-passwordcode')
                <div>
                    <a class="mr-1 btn btn-warning" target="_blank"
                        href="{{ route('passwordcode.export', ['by_date' => $byDate, 'from_date' => $fromDate, 'to_date' => $toDate]) }}">
            ...
            @permission('create-passwordcode')
                <div>
                    <button type="button" class="btn btn-success" data-toggle="modal"
                        data-target="#create-new-passwordcode-modal">
    ...
    <div wire:ignore.self class="modal fade" id="show-hide-passwordcode-columnModal" tabindex="-1"
        aria-labelledby="show-hide-passwordcode-columnModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="show-hide-passwordcode-columnModalLabel">
                ...
                <div class="modal-body">
                    <div>
                        ...
                        @for ($i = 0; $i < count($showColumn); $i++)
                            ...
                            @if (strlen(__('passwordcode.' . $show_kye)) > 2)
                                <div class="form-check">
                                    <input wire:model="showColumn.{{ $kye }}" class="form-check-input"
                                        type="checkbox" value="true" id="filter-passwordcode-{{ $kye }}">
                                    <label class="form-check-label" for="filter-passwordcode-{{ $kye }}">
                                        {{ __('passwordcode.' . $show_kye) }}
                                    </label>
                                </div>
                            @endif
                        @endfor
        ...
        <div wire:ignore.self class="modal fade" id="import-data" tabindex="-1" aria-labelledby="import-dataLabel"
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form action="{{ route('passwordcode.import') }}" enctype="multipart/form-data" method="post"
    ...
    <div wire:ignore.self class="modal fade" id="filter-passwordcode-modal" tabindex="-1"
        aria-labelledby="filter-passwordcode-modalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="filter-passwordcode-modalLabel">
                        {{ __('global.filter') }}
                    </h5>
