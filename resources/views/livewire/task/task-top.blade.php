<div class="d-flex mb-2">
    <div id="search-sort-section" class="form-inline mr-auto">
        ...
        <div>
            <select wire:model='orderBy' class="form-control form-group" style="width: 150px;">
                ...
                <option value='main_task_id'>{{ __('task.main_task') }}</option>
    ...
    <div class="form-inline">
        ...
        @role('owner|operations')
            <div>
                <button type="button" class="btn btn-info" data-toggle="modal" data-target="#show-hide-task-columnModal">
            ...
            @permission('export-excel-task')
                <div>
                    <a class="mr-1 btn btn-warning" target="_blank"
                        href="{{ route('task.export', ['by_date' => $byDate, 'from_date' => $fromDate, 'to_date' => $toDate]) }}">
        ...
        @if ($admin_view_status != 'deleted')
            @permission('create-task')
                <div>
                    <button type="button" class="btn btn-success" data-toggle="modal" data-target="#create-new-task-modal">
    ...
    <div wire:ignore.self class="modal fade" id="show-hide-task-columnModal" tabindex="-1"
        aria-labelledby="show-hide-task-columnModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="show-hide-task-columnModalLabel">
                ...
                <div class="modal-body">
                    <div>
                        ...
                        @for ($i = 0; $i < count($showColumn); $i++)
                            ...
                            @if (strlen(__('task.' . $show_kye)) > 2)
                                <div class="form-check">
                                    <input wire:model="showColumn.{{ $kye }}" class="form-check-input"
                                        type="checkbox" value="true" id="filter-task-{{ $kye }}">
                                    <label class="form-check-label" for="filter-task-{{ $kye }}">
                                        {{ __('task.' . $show_kye) }}
                                    </label>
                                </div>
                            @endif
    ...
    @if ($admin_view_status != 'deleted')
        @include('livewire.task.task-create-modal')
    @endif
        ...
        <div wire:ignore.self class="modal fade" id="import-data" tabindex="-1" aria-labelledby="import-dataLabel"
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form action="{{ route('task.import') }}" enctype="multipart/form-data" method="post"
    ...
    <div wire:ignore.self class="modal fade" id="filter-task-modal" tabindex="-1"
        aria-labelledby="filter-task-modalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="filter-task-modalLabel">
                        {{ __('global.filter') }}
                    </h5>
                ...
                <div class="modal-body">
                    ...
                    {{-- <div class='form-group'>
                        <label for='main_task-select'>{{ __('global.by_main_tasks') }}</label>
                        <select id='main_task-select' class='form-control' wire:model='select_main_task'>
                            <option>Select main_tasks</option>
                            @foreach ($main_tasks as $main_task)
                                <option value='{{ $main_task->id }}'>{{ $main_task->crud_name() }}</option>
                            @endforeach
                        </select>

                        @foreach ($main_tasks as $main_task)
                            @if (in_array($main_task->id, $filter_main_tasks_id))
                                <div class='form-check form-check-inline'>
                                    <input wire:model='filter_main_tasks_id' class='form-check-input' type='checkbox'
                                        value='{{ $main_task->id }}' id='filter-main_tasks-id-{{ $main_task->id }}'>
                                    <label class='form-check-label' for='filter-main_tasks-id-{{ $main_task->id }}'>
                                        {{ $main_task->crud_name() }}
                                    </label>
                                </div>
                            @endif
