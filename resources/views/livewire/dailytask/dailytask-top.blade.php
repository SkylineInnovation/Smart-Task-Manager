<div class="d-flex mb-2">
    ...
    <div class="form-inline">
        @role('owner|operations')
            <div>
                <button type="button" class="mr-1 btn btn-warning" data-toggle="modal"
                    data-target="#filter-dailytask-modal">
        ...
        @if ($admin_view_status != 'deleted')
            ...
            @permission('export-excel-dailytask')
                <div>
                    <a class="mr-1 btn btn-warning" target="_blank"
                        href="{{ route('dailytask.export', ['by_date' => $byDate, 'from_date' => $fromDate, 'to_date' => $toDate]) }}">
    ...
    <div wire:ignore.self class="modal fade" id="show-hide-dailytask-columnModal" tabindex="-1"
        aria-labelledby="show-hide-dailytask-columnModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="show-hide-dailytask-columnModalLabel">
                ...
                <div class="modal-body">
                    <div>
                        ...
                        @for ($i = 0; $i < count($showColumn); $i++)
                            ...
                            @if (strlen(__('dailytask.' . $show_kye)) > 2)
                                <div class="form-check">
                                    <input wire:model="showColumn.{{ $kye }}" class="form-check-input"
                                        type="checkbox" value="true" id="filter-dailytask-{{ $kye }}">
                                    <label class="form-check-label" for="filter-dailytask-{{ $kye }}">
                                        {{ __('dailytask.' . $show_kye) }}
                                    </label>
                                </div>
                            @endif
                        @endfor
    ...
    @if ($admin_view_status != 'deleted')
        @include('livewire.dailytask.dailytask-create-modal')
    @endif
        ...
        <div wire:ignore.self class="modal fade" id="import-data" tabindex="-1" aria-labelledby="import-dataLabel"
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form action="{{ route('dailytask.import') }}" enctype="multipart/form-data" method="post"
    ...
    <div wire:ignore.self class="modal fade" id="filter-dailytask-modal" tabindex="-1"
        aria-labelledby="filter-dailytask-modalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="filter-dailytask-modalLabel">
                        {{ __('global.filter') }}
                    </h5>
